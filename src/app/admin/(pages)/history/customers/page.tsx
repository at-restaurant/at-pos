...
// ✅ CUSTOMERS DATABASE: Delivery & Takeaway customer tracking

'use client'

import { useState, useEffect, useMemo } from 'react'
import { createClient } from '@/lib/supabase/client'
import { ArrowLeft, MapPin, Phone, User, Search, Download, ShoppingBag } from 'lucide-react'
                ...
                <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white shadow-lg">
                            <User className="w-8 h-8 opacity-80 mb-2" />
                        ...
                        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-4 text-white shadow-lg">
                            ...
                            <p className="text-3xl font-bold mt-1">{stats.deliveryCustomers}</p>
                        </div>

                        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-4 text-white shadow-lg">
                            <ShoppingBag className="w-8 h-8 opacity-80 mb-2" />
                            <p className="text-sm opacity-90">Takeaway</p>
                            <p className="text-3xl font-bold mt-1">{stats.takeawayCustomers}</p>
                        </div>

                        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white shadow-lg">
                            <Phone className="w-8 h-8 opacity-80 mb-2" />
                            <p className="text-sm opacity-90">Total Revenue</p>
                            <p className="text-2xl font-bold mt-1">PKR {(stats.totalRevenue / 1000).toFixed(1)}k</p>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-3 bg-[var(--bg)] border border-[var(--border)] rounded-lg text-[var(--fg)] placeholder:text-[var(--muted)] focus:ring-2 focus:ring-blue-600 focus:outline-none"
                                />
                            </div>
                    ...
                    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl overflow-hidden">
                        <div className="p-4 sm:p-6 border-b border-[var(--border)]">
                            <h3 className="font-bold text-[var(--fg)]">All Customers ({filtered.length})</h3>
                        </div>
                                    ...
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="flex items-start gap-3 flex-1 min-w-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="font-semibold text-[var(--fg)] truncate">{customer.name}</p>
                                        ...
                                        <div className="text-right shrink-0">
                                            <p className="font-bold text-blue-600">PKR {customer.totalSpent.toLocaleString()}</p>
                            ...
                            📦 {customer.takeawayCount}
                                    ...
                                    {selectedCustomer === customer && (
                                        <div className="mt-4 pt-4 border-t border-[var(--border)] space-y-2">
                                            <p className="text-sm font-semibold text-[var(--fg)] mb-2">Order History</p>
                                            <div className="space-y-2 max-h-64 overflow-y-auto">
                                                {customer.orders.map((order: any, j: number) => (
                                                    <div key={j} className="p-3 bg-[var(--bg)] rounded-lg text-sm">
                                                        <div className="flex justify-between mb-1">
                                                            <span className="text-[var(--muted)]">#{order.id.slice(0, 8).toUpperCase()}</span>
                                                            <span className="font-semibold text-[var(--fg)]">PKR {order.total_amount.toLocaleString()}</span>
                                                        </div>
                                                        <div className="flex justify-between text-xs">
                                                            <span className="text-[var(--muted)]">{new Date(order.created_at).toLocaleDateString()}</span>
                        ...
                        {filtered.length === 0 && (
                            <div className="p-12 text-center">
                                <User className="w-16 h-16 mx-auto mb-4 opacity-20 text-[var(--fg)]" />
                                <p className="text-[var(--fg)] font-medium">No customers found</p>
