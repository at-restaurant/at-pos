...
// ✅ WAITERS PERFORMANCE: Individual staff tracking with date range

'use client'

import { useState, useEffect, useMemo } from 'react'
import { createClient } from '@/lib/supabase/client'
import { ArrowLeft, Users, TrendingUp, Award, Calendar, Download } from 'lucide-react'
    ...
    const loadData = async () => {
        ...
        try {
            const [waitersRes, ordersRes] = await Promise.all([
                ...
                supabase
                    .from('orders')
                    .select('*, waiters(name, profile_pic)')
                ...
                <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {performance.slice(0, 3).map((waiter: any, i: number) => {
                            ...
                            return (
                                <div key={waiter.id} className={`bg-gradient-to-br ${colors[i].bg} rounded-xl p-6 text-white shadow-2xl relative overflow-hidden`}>
                                    <div className="absolute top-4 right-4 text-6xl font-bold opacity-20">#{i + 1}</div>
                                    <div className="relative">
                                        {waiter.profile_pic ? (
                                            <img src={waiter.profile_pic} alt={waiter.name} className={`w-20 h-20 rounded-full border-4 ${colors[i].border} object-cover mb-4`} />
                                        ) : (
                                            <div className={`w-20 h-20 rounded-full border-4 ${colors[i].border} bg-white/20 flex items-center justify-center text-3xl font-bold mb-4`}>
                                                {waiter.name.charAt(0)}
                                            </div>
                                        )}
                                        <h3 className="text-xl font-bold mb-2">{waiter.name}</h3>
                                        <p className="text-3xl font-bold mb-1">PKR {(waiter.revenue / 1000).toFixed(1)}k</p>
                                        <p className="text-sm opacity-90">{waiter.orders} orders • Avg: PKR {Math.round(waiter.avgOrder)}</p>
                                    </div>
                                </div>
                            )
                        })}
                    ...
                    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl overflow-hidden">
                        <div className="p-4 sm:p-6 border-b border-[var(--border)]">
                            <h3 className="font-bold text-[var(--fg)] flex items-center gap-2">
                        ...
                        <div className="divide-y divide-[var(--border)]">
                            {performance.map((waiter: any) => (
                                <button
                                    ...
                                    className="w-full p-4 hover:bg-[var(--bg)] transition-colors text-left"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        {/* Profile */}
                                        {waiter.profile_pic ? (
                                            <img src={waiter.profile_pic} alt={waiter.name} className="w-12 h-12 rounded-full object-cover border-2 border-blue-600 shrink-0" />
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-[var(--fg)] truncate">{waiter.name}</p>
                                            <p className="text-sm text-[var(--muted)]">{waiter.employee_type}</p>
                                        </div>

                                        {/* Stats */}
                                        <div className="text-right shrink-0">
                                            <p className="text-lg font-bold text-blue-600">PKR {(waiter.revenue / 1000).toFixed(1)}k</p>
                                        ...
                                        <div className="mt-4 pt-4 border-t border-[var(--border)] space-y-3">
                                            <div className="grid grid-cols-3 gap-3">
                                                <div className="p-3 bg-[var(--bg)] rounded-lg">
                                                    ...
                                                    <p className="text-xl font-bold text-[var(--fg)]">{selectedWaiterData.orders}</p>
                                                </div>
                                                <div className="p-3 bg-[var(--bg)] rounded-lg">
                                                    <p className="text-xs text-[var(--muted)]">Total Revenue</p>
                                                    <p className="text-xl font-bold text-green-600">PKR {(selectedWaiterData.revenue / 1000).toFixed(1)}k</p>
                                                </div>
                                                <div className="p-3 bg-[var(--bg)] rounded-lg">
                                                    <p className="text-xs text-[var(--muted)]">Avg Order</p>
                                                    <p className="text-xl font-bold text-blue-600">PKR {Math.round(selectedWaiterData.avgOrder)}</p>
                                                </div>
                                            </div>

                                            {selectedWaiterData.dailyData.length > 0 && (
                                                <div>
                                                    <p className="text-sm font-semibold text-[var(--fg)] mb-2 flex items-center gap-2">
                                                        <Calendar className="w-4 h-4" />
                                                        Daily Performance
                                                    </p>
                                                    <div className="space-y-2 max-h-48 overflow-y-auto">
                                                        {selectedWaiterData.dailyData.map((day: any, i: number) => (
                                                            <div key={i} className="flex justify-between text-sm p-2 bg-[var(--bg)] rounded">
                                                                <span className="text-[var(--muted)]">{day.date}</span>
                                                                <span className="font-semibold text-[var(--fg)]">
                                  {day.orders} orders • PKR {day.revenue.toLocaleString()}
                                </span>
                                                            </div>
                                                        ))}
                                                    </div>
